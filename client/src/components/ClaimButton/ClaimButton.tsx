...
export const ClaimButton = ({signerCanClaim, claimed, isRandom, tokenId, txHash, claimPrice, onClaim, onClaimOther}: IClaimedButtonProps) => {
  ...
  if (claimed && tokenId) {
    return <a href={`https://opensea.io/assets/${deployments.contracts.SyntheticLootCharacter.address}/${tokenId.toString()}`}
      ...
      if (!isRandom) {
        ...
        return <div style={{display:"flex"}}>
          ...
          <button className={`${style.helpBtn} ${style.toolTip}`}>?
            <span className={style.toolTipText}>You may claim this character and have it sent to your connected wallet </span>
          </button>
        </div> 
      }
    }
